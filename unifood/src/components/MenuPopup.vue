<template>
    <div class="fixed inset-0 hidden bg-gray-500/75 z-10 w-screen overflow-y-auto md:block" role="dialog"
                ...
                <div class="grid w-full grid-cols-1 items-stretch gap-x-6 gap-y-8 sm:grid-cols-12 lg:gap-x-8">
                    <div v-if="menu_data?.image" class="sm:col-span-4 lg:col-span-5 relative">
                        <img :src="menu_data.image" alt="Добавьте сюда ваш логотип"
                            class="w-full h-full rounded-lg bg-gray-100 object-cover">
                        <input @change="setImage" id="rela-file" type="file" class="hidden absolute bottom-0" />
                    </div>
                    ...
                    <div class="sm:col-span-8 lg:col-span-7">
                        <input v-model="menu_data.name" placeholder="Задайте название меню" id="information-heading"
                            class="w-1/2 text-3xl font-boldvtext-gray-900 sm:text-4xl" />
                        <input type="text" v-model="menu_data.address" placeholder="Адрес заведения, с данным меню"
                            class="mt-6 w-1/2 text-3xl font-bold text-gray-900 sm:text-4xl" />
                        <textarea rows="4" v-model="menu_data.description" placeholder="Доп описание для меню"
                            class="mt-6 w-auto text-gray-900 sm:text-4xl"></textarea>
                        <p class="mt-6 w-1/2 text-3xl font-bold text-gray-900 sm:text-4xl">Ваш QR код</p>
                        <div><qrcode-vue :value="qrcode" size="200" margin="0"></qrcode-vue>
                        </div>
                        ...
                        <button v-if="!menu_data.is_new" @click="delete_menu" type="submit"
...
export default {
    ...
    data() {
        return {
            ...
            qrcode: window.location.hostname+'/menu-id/'+this.menu_data.id,
        }
    },
    mounted() {
